...
async fn test_creating_admin_user() -> Result<()> {
    ...
    let admin_id = user_service::insert(User::new("admin".to_string(), "password", false)?, &ctx.db_conn).await?;
    ...
    let user_id = user_service::insert(User::new("not_admin".to_string(), "password", false)?, &ctx.db_conn).await?;
