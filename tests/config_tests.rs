...
mod config_integration {
    ...
    async fn get_nothing() {
        let ctx = lib::DbContext::new("get_nothing");
        let pool = ctx.db_pool.expect("Invalid DB connection pool");

        assert!(
            config_service::get_config_value("should-be-nothing".to_string(), pool.clone()).await.is_err(),
    ...
    async fn set_get_something() {
        let ctx = lib::DbContext::new("set_get_something");
        let pool = ctx.db_pool.expect("Invalid DB connection pool");
