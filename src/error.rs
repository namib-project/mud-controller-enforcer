...
pub enum Error {
    ...
    #[snafu(display("RingError: {}", source), context(false))]
    RingError { source: ring::error::Unspecified, backtrace: Backtrace },
    ...
    fn respond_to(self, request: &Request) -> rocket::response::Result<'r> {
        ...
        match self {
            Error::ResponseError { status, message, .. } => {
                ...
                Response::build_from(Json(ErrorDto { error: message }).respond_to(&request)?)
                    .status(status)
                    .ok()
            }
            _ => {
                Response::build_from(Json(ErrorDto { error: String::from("An error occurred") }).respond_to(&request)?)
